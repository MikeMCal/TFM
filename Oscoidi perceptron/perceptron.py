...
class Perceptron():
	...
	def __init__(self):
		...
		self.cwd = os.getcwd()
		self.saveurl = self.cwd + '\TFM Code TESTs\Oscoidi perceptron\perceptron.txt' #url donde guardaremos la red neuronal una vez entrenada
		self.trainingdata = self.cwd + '\TFM Code TESTs\Oscoidi perceptron\entrenamientoperc.txt' #url donde tenemos la training data
	...
	def train(self): #funcion que inicia el entrenamiento
		...
		for r in range(int(input('numero de veces que quiere repetir el entrenamiento: '+ str(self.ntrain)))):
			self.readlines()
		self.save()
